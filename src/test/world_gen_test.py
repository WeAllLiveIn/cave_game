...
class WorldMap:
    ...
    def generate_obstruction(self, obstructions: int = 0) -> None:
    ...
    def choose_first_room(self) -> None:
        x, y = randrange(0, self.mapsize), randrange(0, self.mapsize)
        self.map[y][x]: Room = Room((x, y))
        self.map[y][x].exit = True
        self.start_room_xy: tuple = (x, y)
        self.rooms_list += [(x, y)]

    def get_random_xy(self) -> tuple:
        x, y = choice(self.rooms_list)
        xy = tuple(filter(lambda m: 0 <= m[0] < self.mapsize and 0 <= m[1] < self.mapsize and self.map[m[0]][m[1]] == 0,
                          [(x + 1, y), (x - 1, y), (x, y - 1), (x, y + 1)]))
        return xy

    def generate_new_worldmap(self, mapsize: int = 3, rooms_limit: int = 3, blocks: int = 0) -> None:
    ...
    def __getitem__(self, xy: tuple[int]) -> Room:
        x, y = xy
        return self.map[y][x]
    ...
    def __setitem__(self, xy: tuple[int], item) -> None:
        x, y = xy
        self.map[y][x] = item

    def __delitem__(self, xy: tuple[int]) -> None:
        self.map[y][x] = 0
